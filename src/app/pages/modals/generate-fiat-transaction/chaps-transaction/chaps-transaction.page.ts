...
@Component({
  selector: 'app-chaps-transaction',
  templateUrl: './chaps-transaction.page.html',
  styleUrls: ['./chaps-transaction.page.scss']
})

export class ChapsTransactionPage implements OnInit, OnDestroy {
  private subs: Subscription[] = [];
  public fromAddress: string;
  public currencyCode: string;
  public amount: number;

  public chapsTransactionForm: FormGroup;
  ...
  constructor(
    ...
    this.chapsTransactionForm = this.formBuilder.group({});
  }
  ...
  async ngOnInit() {
    ...
    this.getChapsRequisites(loading);
  }
  ...
  private getChapsRequisites(loading) {
    this.fiatTransactionsService.getFiatTransactionsRequisites('chaps').pipe(
      map((data: any) => data.data)
    ).subscribe(
      async (data) => {
        for (const [key] of Object.entries(data)) {
          this.chapsTransactionForm.addControl(key, new FormControl(null, Validators.required));
        }
  ...
  public async createChapsTransaction() {
    const loading = await this.loadingCtrl.create({ spinner: 'crescent' });
    const formValue = this.chapsTransactionForm.value;
    ...
    await loading.present();
    const subCreateSwift = this.fiatTransactionsService.createChapsTransaction(body).subscribe((res: any) => {
